...
namespace Ict\ArticleUrl\Service;
...
class ServiceDecorator implements RouterInterface, WarmableInterface
{
    ...
    {
        $subpath = $this->systemConfigService->get('IctArticleUrl.config.subpath');
        if (strlen((string) $subpath) > 0) {
            ...
            if (substr($pathinfo, 0, strlen($subpath)) === $subpath) {
                ...
                $products = $this->productRepository->search(
                    (new \Shopware\Core\Framework\DataAbstractionLayer\Search\Criteria())->addFilter(new \Shopware\Core\Framework\DataAbstractionLayer\Search\Filter\EqualsFilter('productNumber', urldecode(substr($pathinfo, strlen($subpath))))),
