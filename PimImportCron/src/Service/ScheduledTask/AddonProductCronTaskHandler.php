...
class AddonProductCronTaskHandler extends ScheduledTaskHandler
{
    ...
    {
        return [ AddonProductCronTask::class ];
    }
    ...
    public function run(): void
    {
        ...
        for ($i=1; $i<=$last_counter; $i = $i+50) {
            ...
            if (isset($response->Products)) {
                file_put_contents("AddOnImportLog.txt", date("l jS \of F Y h:i:s A")."> Start Cron Addon Product Import\n", FILE_APPEND);
                foreach ($response->Products as $res) {
                    echo $counter.'--';
                    file_put_contents("AddOnImportLog.txt", date("l jS \of F Y h:i:s A")."> ".$counter." Addon Product Import\n", FILE_APPEND);
                    $crossSellingID = '';
                    $flag = false;
                        ...
                        if (empty($checkUpdatedProduct)) {
                            ...
                            if ($crossSelling) {
                                foreach ($crossSelling as $crossData) {
                                    if ($crossData->getName() == 'Addon Products') {
                            ...
                            if ($crossSellingID) {
                                ...
                                if ($res->AddOnProducts) {
                                    $i = 1;
                                    foreach ($res->AddOnProducts as $addonProduct) {
                                        $addonNumber = $addonProduct->AddOnCode->Value;
                                        $addonNumberSequenceNo = $addonProduct->SequenceNo->Value;
                                        if (!empty($this->checkProductNumber($addonNumber))) {
                                            $addonID = $this->getProductData($addonNumber, $context)->getId();
                                            if ($addonID) {
                                                $i++;
                                                $data = [
                                                    'crossSellingId' => $crossSellingID,
                                                    'productId' => $addonID,
                                                    'position' => $addonNumberSequenceNo?$addonNumberSequenceNo:$i,
                                                ];
                                                $this->productCrossSellingAssignedProductsRepository->upsert([$data], $context);
                                                $flag = true;
                                            }
                                        }
                                    }
                                ...
                                if ($flag == true) {
                                    ...
                                    $pimData = [
                                        ...
                                        'lastAddonCrossSellUsage' => $checkDate,
                                    ];
                ...
                file_put_contents("AddOnImportLog.txt", date("l jS \of F Y h:i:s A")."> End Cron Addon Product Import\n", FILE_APPEND);
            }
        }
    }
    ...
    public function getCurlData($apiUrl)
    {
        ...
        curl_close($curl);
        if ($err) {
            echo 'Error Curl Time Out Addon Product Import';
            file_put_contents("AddOnImportLog.txt", date("l jS \of F Y h:i:s A")."> Error Curl Time Out Addon Product Import\n", FILE_APPEND);
    ...
    public function checkUpdatedProduct($date = null, $productNumber = null): ?string
    {
        ...
        $criteria->addFilter(new EqualsFilter('lastAddonCrossSellUsage', $date));
