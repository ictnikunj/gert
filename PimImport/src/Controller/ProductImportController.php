...
class ProductImportController extends AbstractController
{
    ...
    private $acrisDownloadRepository;
    ...
    /**
     * @Route("/api/pim/productimport", name="api.action.pim.product.import", methods={"GET"})
     */
    public function productImport(Context $context, $val = null): JsonResponse
    {
        ...
        $mainProductCounter = $this->systemConfigService->get('PimImport.config.mainProductCounter');
        ...
        if ($err) {
            ...
            curl_setopt_array($curl, array(
                ...
                //CURLOPT_URL => 'https://fluidmaster.compano.com/api/jsonfeed/FM_Web_PRODUCT/1/100?filter=Code=675016&apikey=8A0F889905CF2C37A092B9A0C80DAEFC38E7408D188BB02A6ECF8BE6A329A690',
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => "",
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 30,
...
//                                        $imageFlag = true;
                            ...
                            if ($res->Items) {
                                $salesChannelObject = $this->removeSalesChannel($productData, $context);
                ...
                $this->systemConfigService->set('PimImport.config.mainProductCounter', $counter);
    ...
    public function removeSalesChannel($productData, $context)
    {
        $criteria = new Criteria();
        $criteria->addFilter(new EqualsFilter('productId', $productData['id']));
        $sales_object = $this->productVisibilityRepository->search($criteria, $context);
        if ($sales_object->getTotal() > 0) {
            foreach ($sales_object->getElements() as $sale) {
                $this->productVisibilityRepository->delete([[
                    'id' => $sale->getId()
                ]], $context);
            }
        }
...
//        dd($data);
    ...
    // private function removeMediaId (string $productId,string $mediaId, Context $context)
    // {
    //     $criteria = new Criteria();
    //     $criteria->addFilter(new EqualsFilter('fileName',$fileName));
    //     $media_object = $this->mediaRepository->searchIds($criteria, $context);
...
//    private function checkProductImageExist (string $productId,string $mediaId, Context $context)
//    {
//        $criteria = new Criteria();
